...
*   @returns {object} analyzed Transaction objects
**/
api.prototype.sendTransfer = function(depth, minWeightMagnitude, transfers, options, callback) {
    ...
    self.prepareTransfers(transfers, options, function(error, trytes) {
...
api.prototype._generateKey = function (filename, security) {
    if (fs.existsSync(filename)) {
        fs.renameSync(filename, filename + '_old');
    }
    var tritstr = "ABCDEFGHIJKLMNOPQRSTUVWXYZ9";
    var secureRandom = require('secure-random');
    var rn = secureRandom.randomUint8Array(security * 243 * 9);
    var keystr = '';

    for (var i = 0; i < rn.length; i++ ){
        var num = rn[i] % 27;
        keystr += tritstr.substr(num, 1);
    }

    fs.writeFileSync(filename, keystr);

    return Converter.trits(keystr);
};

api.prototype._readKey = function (filename) {
    var keystr = fs.readFileSync(filename, 'utf-8');
    return Converter.trits(keystr);
};

api.prototype._readSecurityCount = function (filename) {
    try {
        var stat = fs.statSync(filename);
        return stat.size / (243 * 9);
    } catch (err) {
        return 1;
    }
}
...
**/
api.prototype._newAddress = function(security, checksum) {

    var key = this._generateKey(this.keypath, security);
...
api.prototype._getAddress = function(filename, checksum) {
    var key = this._readKey(filename);
    var digests = Signing.digests(key);
    var addressTrits = Signing.address(digests);
    var address = Converter.trytes(addressTrits)
...
**/
api.prototype.getNewAddress = function(options, callback) {
    // If no options provided, switch arguments
    if (arguments.length === 1 && Object.prototype.toString.call(options) === "[object Function]") {
        callback = options;
        options = {};
    }
    ...
    var newAddress = this._newAddress(security, checksum);
...
*   @param {function} callback
**/
api.prototype.getInputs = function(options, callback) {
    ...
    if (options.keypaths) {
        ...
        for (var i = 0; i< options.keypaths.length; i++) {
            var keypath = options.keypaths[i];
            addresses.push(this._getAddress(keypath, false));
        }
    } else {
        addresses.push(this._getAddress(this.keypath, false));
        keypaths = [this.keypath];
    }
                ...
                for (var i = 0; i < addresses.length; i++) {
                    ...
                    if (balance > 0) {
                        ...
                        var newEntry = {
                            ...
                            'keyIndex': i,
                            'keypath' : keypaths[i],
                            'security' : self._readSecurityCount(keypaths[i])
                        }
...
**/
api.prototype.prepareTransfers = function(transfers, options, callback) {
    ...
    if (totalValue) {
        ...
        //
        //  If no inputs provided, derive the addresses from the secret key and
        //  confirm that the inputs exceed the threshold
        else {
            var keypaths = [ self.keypath ];

            if (fs.existsSync(self.keypath + '_old')) {
                keypaths.push(self.keypath + '_old');
            }

            self.getInputs({ 'keypaths': keypaths,  'threshold': totalValue, 'security': security }, function(error, inputs) {
        ...
        for (var i = 0; i < inputs.length; i++) {
            ...
            if (thisBalance >= totalTransferValue) {
                ...
                if (remainder > 0 && remainderAddress) {
                    ...
                    return callback(new Error("Not Specify Remainder Address"));

                    /*
                    // Generate a new Address by calling getNewAddress
                    self.getNewAddress({'security': security}, function(error, address) {
...
*   @returns {object} success
**/
api.prototype.getTransfers = function(options, callback) {
    ...
    var address = this._getAddress(this.keypath, false);
    var addresses = [ address ];

    self._bundlesFromAddresses(addresses, inclusionStates, callback);
}
...
*   @returns {object} success
**/
api.prototype.getAccountData = function(options, callback) {
    ...
    //  @addresses:     all addresses associated with secret key
    ...
    valuesToReturn.addresses = addresses;
